 ...
 */ export function bufferIndexOf(buf, sig) {
 ...
 */ export function findDeflateBoundary(combined, isZip64) {
    // Look for next structure signature (definitive end markers)
    let boundaryPos = -1;
    for (const sig of [
        C.SIG_LOCAL_FILE,
        C.SIG_CENTRAL_DIR,
        C.SIG_END_OF_CENTRAL_DIR
    ]){
 ...
 */ export function findStoreDataEnd(buffer, isZip64) {
 ...
 * or next header.
 */ export function getSafetyBufferSize() {
